...
impl LaunchManager {
    ...
    pub fn enable_launch_at_login(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.auto_launch.enable().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
    }

    pub fn disable_launch_at_login(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.auto_launch.disable().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
    }

    pub fn is_launch_at_login_enabled(&self) -> Result<bool, Box<dyn std::error::Error>> {
        self.auto_launch.is_enabled().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
    }
